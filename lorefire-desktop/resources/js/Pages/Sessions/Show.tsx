...
export default function Show({ campaign, session, characters, transcriptSegments, speakerProfiles, campaignVoiceprints = [], imageGenProvider, recoverableTakes = [] }: Props) {
  const {
    isRecording,
    recordingSeconds,
    isUploading,
  ...
  const [remapLine, setRemapLine] = useState<{ indexes: number[]; label?: string } | null>(null)
  ...
  return (
                    ...
                    <div className="max-h-96 overflow-y-auto flex flex-col gap-0">
                      {transcriptSegments.map((seg, i) => (
                        ...
                        >
                          ...
                          {/* Dialogue */}
                          <span className="text-xs text-[var(--color-text-base)] leading-relaxed flex-1">
                            {seg.text.trim()}
                          </span>

                          {seg.speaker_label && (
                            <ChangeSpeakerButton
                              onClick={() => setRemapLine({
                                indexes: [typeof seg.segment_index === 'number' ? seg.segment_index : i],
    ...
    {remapLine && transcriptSegments && (
      <SpeakerRemapDialog
        sessionId={session.id}
        segmentIndexes={remapLine.indexes}
        currentLabel={remapLine.label}
        speakerProfiles={speakerProfiles}
        campaignVoiceprints={campaignVoiceprints}
        characters={characters}
        transcriptSegments={transcriptSegments}
